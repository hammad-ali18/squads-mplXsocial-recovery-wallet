...
/// Adding SRW feature, to restore the wallet
#[account]
pub struct Ms {
...
impl Ms {
    ...
    ///Checks to see the member is a part of recovery
    pub fn is_recovery_member(&self, initiator: Pubkey) -> Option<usize> {
        match self.recovery_keys.binary_search(&initiator) {
            Ok(index) => Some(index),
           Err(_) => None,
        }
    }
    ...
    pub fn add_member_to_recover_wallet(&mut self, member: Pubkey) -> Result<()> {
        if matches!(self.is_recovery_member(member), None) {
            self.recovery_keys.push(member);
            self.recovery_keys.sort();
        }
        Ok(())
    }
    //initiate recovery
    pub fn initiate_recovery(&mut self,initiator: Pubkey)->Result<()>{
        if self.is_recovery_member(initiator).is_none() {
            return Err(MsError::UnAuthorizedRecover.into());
        }
        self.recovery_initiated=true;
        self.recovery_approvals.checked_add(0).unwrap();
        Ok(())
    }
    ...
    /// Removes a member from the multisig. and updates the threshold if successfully removed
